...
pub async fn build_router(
    ...
    let app = Router::new()
        ...
        // QR login flow (WhatsApp / WeChat)
        .route(
            "/api/channels/{name}/qr/start",
            axum::routing::post(routes::channel_qr_start),
        )
        .route(
            "/api/channels/{name}/qr/status",
            axum::routing::get(routes::channel_qr_status),
        )
        // Template endpoints
